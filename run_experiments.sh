...
LB_URL="${LB_URL:-http://127.0.0.1:${LB_PORT}}"
...
start_load_balancer() {
    ...
    for _ in {1..30}; do
        if curl --fail --silent --show-error --max-time 2 "${LB_URL}/lb/health" >/dev/null 2>&1; then
            return
        fi
...
run_experiment() {
    ...
    curl --fail --silent --show-error "${LB_URL}/lb/status" | tee "${name}_lb_status.json"
    echo
    ...
    curl --fail --silent --show-error "${LB_URL}/lb/metrics" | tee "${name}_lb_metrics.json"
    echo
    stop_load_balancer
}
